  ...
  render() {
    ...
    return html`<ha-card>
      ...
      ${hasUpdate ? html`<section class="update-banner"><div class="update-head"><div><div class="update-title"><ha-icon icon="mdi:update"></ha-icon>${t.updateReady}</div><div class="latest">${t.latest}: ${attributes.latest_version ?? "—"}</div></div>
        <ha-progress-button appearance="filled" data-action="update" .progress=${this._operation === "update"} ?disabled=${Boolean(this._operation)} @click=${() => this._run("update")}><ha-icon icon="mdi:download"></ha-icon>${t.update}</ha-progress-button></div>
      ...
      ${this._error ? html`<div class="error" role="status" aria-live="polite">${this._error}</div>` : nothing}</section>
      <div class="actions">
